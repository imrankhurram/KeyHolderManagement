...
//	}
	...
	public void updateUsersActivity() { // overloading
		 ...
		 */
		/*
		 * this.setAlmsPerHour(); this.setActionedAlms();
		 * this.setAlmsLastYear(); this.setAvgResponseTime();
		 */
...
//	}
	...
	public String loginAction() {
		...
		if (mUserName.equalsIgnoreCase("user")
				&& mPassword.equalsIgnoreCase("password")) {
	...
	public String action() {
		...
		FacesMessage message = new FacesMessage("Clicked on User: "
				+ getUserName());
...
//	}
	...
	public void dbInsertLogin() {
		UserActivityDAO.getInstance().userLogin(getUserName(),
...
//		return processLoginUsers();//for only users management system standalone app
	...
	public String processLoginUsers() {
		if ((UserDAO.getInstance().correctPassword(this.getUserName(),
			...
			session.setAttribute("usersOnly", Boolean.TRUE);
	...
	public String processLoginKeyholders() {
		if ((UserDAO.getInstance().correctPassword(this.getUserName(),
				encryptPassword(this.getPassword())) == true)) {
			User currentUser = UserDAO.getInstance().getSpecificUser(
					this.getUserName());
			...
			session.setAttribute("name",
					this.getFirstName() + " " + this.getLastName());
			session.setAttribute("userType", this.getUserType());
			session.setAttribute("usersOnly", Boolean.FALSE);
			this.updateUsersActivity();
			this.loginAudit();
			wrongDetails = false;
			...
			// List<Website>
			// websites=WebsiteDAO.getInstance().getAssignedWebsites(22);
			...
			if (websitesPageBean.getWebsitesList().size() == 1) {
				...
				if (departmentsPageBean != null) {
					...
					List<String> branchCodes = WebsiteDAO.getInstance()
							.getBranchCodes(
									websitesPageBean.getWebsitesList().get(0)
											.getWebsiteId());
